...
class CigarettesHandler extends ConfigurationParamsHandler
{
    const CATEGORY_NAME = 'cigarettes';
    ...
    {
        $cigarettes = $this->em->getRepository(FlexParam::class)->findByCategoryAndFacilityLayout(
            $facilityLayout,
            $this->em->getRepository(AccountingCategory::class)->findOneBy(
                [
                    'key' => FlexParamService::ACCOUNTING_CATEGORY_CIGARETTES_KEY
                ]
            ),
            'backoffice'
        );

        if ($cigarettes && !isset($requestData['cigarettes'])) {
            return true;
        }
        ...
        if (isset($cigarettes) && isset($requestData['cigarettes'])) {
            /** @var FlexParam $cigarette */
            foreach ($cigarettes as $cigarette) {
                $key = $cigarette->getKey();
                $accountingPosition = $cigarette->getAccountingPosition()->getId();

                if (isset($requestData['cigarettes'][$accountingPosition][$key])) {
                    if ($cigarette->getValue() != $requestData['cigarettes'][$accountingPosition][$key]) {
                        return true;
                    }
                } else {
                    return true;
    ...
    public function addPositions(array $requestData, FacilityLayout $facilityLayout, $update = false)
    {
        ...
        /** @var AccountingCategory $cigarettesCategory */
        $cigarettesCategory = $this->em->getRepository(AccountingCategory::class)->findOneBy(
            [
        ...
        foreach ($config['AccountingPositions'] as $category) {
            ...
            if ($key == static::CATEGORY_NAME) {
                ...
                foreach ($requestData[static::CATEGORY_NAME] as $row) {
                    $accountingPosition = $this->addAccountingPosition($cigarettesCategory, $facilityLayout, $sequence);
